...
int main() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Replace this explicit rendering of the Earth and Moon
    // with a traversal of the scene graph and rendering of all its
    // nodes.
    std::stack<CelestialBodyRef> Cstack;
    ...
    while (!Cstack.empty()) {
      CelestialBodyRef planet = Cstack.top();
      Cstack.pop();

      glm::mat4 currt = planet.parent_transform;

      glm::mat4 transform = planet.body->render(
          animation_delta_time_us, camera.GetWorldToClipMatrix(),
          planet.parent_transform, show_basis);
